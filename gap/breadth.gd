...
#! The algebra $L$ is nilpotent if there exists $c \in \N$ such that $L^{c+1} = {0}$, and the minimal such $c$ 
#! is called the class $\mathrm{cl}(L)$ of $L$. For a nilpotent Lie algebra $L$, the type of $L$
#! is the vector $(d_1,\dots,d_c)$, where $d_i = \dim L^i$. A nilpotent Lie algebra $L$ is said to be of maximal class if
...
#! with entries $c_{ijk}$, these matrices are the structure matrices of $L$. For $x = x_1 b_1 + \ldots + x_n b_n \in L$,
...
#! $n \times n$ matrix over $K$ whose $k$-th column is $C_k \overline{x}^{tr}$, this is the adjoint matrix of $L$ with respect to the basis $B$. 
...
#! **i.e.** $[L_i,L_j]\subseteq L_{i+j}$.}. Any nilpotent Lie algebras can be graded by taking $L_i = \gamma_i(L)/\gamma_{i+1}(L)$.
...
#! dimension $p$. The algebra $A$ is an associative, commutative and has a unit. The algebra $L\otimes A$ over $K$ 
...
#! derivation of $M^\uparrow$ of degree 1. Define the derivation $E\in \Der M^\uparrow$ by 
#! $E(x\otimes \varepsilon^i) = D^\uparrow(x\otimes\varepsilon^i\cdot\varepsilon^{p-1})+1\otimes\partial_\varepsilon(\varepsilon^i)$.
#! Let $s\in L_1\setminus M$, take $D=\mathrm{ad}_s$, and extend it naturally to 
#! $M^\uparrow$. Denote $E_{s^\prime}$ as previously. The **inflation** ${}^{M}L$ of $L$ at $M$ by 
...
#! For n=3 one can directly load the Lie algebra by reading the file "inflation_3.g".
#! @Arguments n
